-- Allow an 'unsold' player to be brought back into auction directly from the
-- Unsold / Re-auction tab, without requiring the auctioneer to first flip the
-- player's status in the Players list. On entry we promote the player to
-- 'reauction' so downstream logic (queue row status, place_bid's reauction
-- floor of min_player_price) fires automatically.
...
begin
  ...
  if v_player_status not in ('ready_for_auction', 'reauction', 'unsold') then
  ...
  -- Unsold players re-enter as reauction so the lower bid floor applies.
  v_start_status := case
    when v_player_status = 'ready_for_auction' then 'ready_for_auction'
    else 'reauction'
  end;
  ...
  -- Ensure this player exists in queue before promoting to current.
  if not exists (
    ...
    values (
      v_auction_id,
      p_player_id,
      (select category from public.players where id = p_player_id),
      v_next_order,
      case when v_start_status = 'reauction' then 'reauction' else 'pending' end
    )
  ...
  set status = case
    when v_start_status = 'reauction' then 'reauction'
    else 'in_auction'
  end
