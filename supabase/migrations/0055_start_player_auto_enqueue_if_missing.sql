-- Allow eligible players (ready/reauction) to be started even if
-- they do not currently have an auction_queue row.
...
begin
  ...
  if not exists (
    ...
    values (
      ...
      v_next_order,
      case when v_player_status = 'reauction' then 'reauction' else 'pending' end
    )
